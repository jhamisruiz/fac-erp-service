...
$command
    ...
    ->setCode(function (InputInterface $input, OutputInterface $output) {
        ...
        echo "$base/{$componentName}" . $ns . ".php";
        file_put_contents(
            "$base/$componentName" . $ns . ".php",
            $generated
        );
        file_put_contents("$base/$componentName" . $ns . ".php", $generated);

        //|--------------------------------------------------------------------------
        //|Agregamos la estructura MVC
        //|--------------------------------------------------------------------------
        $directories = ['Domain', 'Http', 'Persistence'];
        foreach ($directories as $directory) {
            if ($directory == 'Domain') {
                //models
                mkdir("$base/$directory/Models", 0755, true);
                echo "\n$base/$directory/Models/{$componentName}" . ucfirst($directory) . ".php";
                $model = $twig->load('Mnt/Domain/Models/models.twig');
                $mdl = $model->render([
                    'name' => ucfirst($componentName),
                    'class' => ucfirst($componentName) . ucfirst('Models'),
                    'ns' => $ns,
                    'namespace' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\' . ucfirst($directory) . '\\' . ucfirst('Models'),
                ]);
                file_put_contents("$base/$directory/Models/{$componentName}Models.php", $mdl);

                //response
                mkdir("$base/$directory/Response", 0755, true);
                echo "\n$base/$directory/Response/{$componentName}" . ucfirst($directory) . ".php";
                $model = $twig->load('Mnt/Domain/Response/response.twig');
                $mdl = $model->render([
                    'name' => ucfirst($componentName),
                    'class' => ucfirst($componentName) . ucfirst('Response'),
                    'ns' => $ns,
                    'namespace' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\' . ucfirst($directory) . '\\' . ucfirst('Response'),
                ]);
                file_put_contents("$base/$directory/Response/{$componentName}Response.php", $mdl);
                //repo
                mkdir("$base/$directory/Repository", 0755, true);
                echo "\n$base/$directory/Repository/{$componentName}" . ucfirst($directory) . ".php";
                $model = $twig->load('Mnt/Domain/Repository/repository.twig');
                $mdl = $model->render([
                    'name' => ucfirst($componentName),
                    'class' => ucfirst($componentName) . ucfirst('Repository'),
                    'ns' => $ns,
                    'namespace' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\' . ucfirst($directory) . '\\' . ucfirst('Repository'),
                    'persistence' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName,
                    'response' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\Domain',
                ]);
                file_put_contents("$base/$directory/Repository/{$componentName}Repository.php", $mdl);
            }
            ...
            if ($directory === 'Http') {
                ...
                $gr = $controller->render([
                    ...
                    'repository' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\Domain',
                    'models' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\Domain',
                ]);
        ...
        $nuevaLinea = '        ' . $componentName . $ns . '::Create($router);';
        ...
        $use = "use " . $ns . "\\" . ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . "\\" . $componentName . $ns . ";";
